...
struct DataService {
  ...
  var data = [City]()
  static let dataPreview = City(name: "Tokyo",
  ...
  mutating func getFileData() {
    // We need to get the file path to DemoData.json
    if let url = Bundle.main.url(forResource: "DemoData", withExtension: "json") {
      ...
      do {
        ...
        do {
          self.data = try decoder.decode([City].self, from: data)
        } catch {
